...
async def test_alcohol_before_diving_question_answers_policy_not_medical_escalation():
    ...
    directo, sin escalar (a diferencia de la cascada de pre_gadea, el
    router del grafo SÍ invoca `detect_routing_signals` para clasificar la
    ruta antes de llegar a este gate determinista — la garantía real es
    que el resultado final no escale, no que el LLM nunca se llame)."""
    state = make_state()
