...
_PRICE_QUESTION = re.compile(
    ...
    # Auditoría 2026-08-26 (Grupo 5): "¿qué es más barato, X o Y?" es una
    # pregunta de precio tan real como "¿cuánto cuesta?" pero no usa
    # ninguna de esas palabras — sin esto, `_canonical_price_named_services_
    # answer` nunca llegaba a evaluarse para una comparación de precio.
...
# Auditoría 2026-08-26 (batería sintética contra PRE, Grupo 5): el comentario
# de `_PRICE_QUESTION` decía "una pregunta que SÍ nombra un servicio se deja
# a RAG, que la responde bien" — medido en vivo que es FALSO para una
# pregunta de precio "en frío" (sin contexto de reserva ya establecido):
# "cuánto cuesta el buceo certificado en dólares?" y "cuánto es el snorkel
# en pesos colombianos?" fallaban el grounding del RAG (`ungrounded_amount`/
# `HALLUCINATED`) y caían al fallback genérico "no lo tengo a la mano" —
# pese a que el precio SÍ está disponible en el mismo catálogo `SERVICES`
# que ya alimenta `_canonical_price_overview_answer`. En vez de depender de
# una retrieval que se ha medido poco fiable para este caso muy concreto y
# bien definido, se responde con el mismo dato determinista — pero solo
# cuando la pregunta nombra EXACTAMENTE uno de los 4 servicios del catálogo
# (buceo certificado/minicurso/snorkel/open water) sin ambigüedad; cualquier
# otra cosa (comida, hotel, buceo nocturno, paquetes multi-día, curso
# specialty...) NO está en este catálogo y sigue yendo a RAG como antes.
