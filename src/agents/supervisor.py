...
# Hallazgo (batería sintética contra PRE, 2026-08-26, lote 4): pregunta
# directa de si se puede tomar alcohol antes de bucear — respuesta plana ya
# conocida (`policies.json["no_alcohol_policy"]`), no un tema médico a
...
def _in_active_cart_building(state: ConversationState) -> bool:
    ...
    tiempo siendo código muerto (sus propios tests de regresión,
    `test_multiday_switch_by_text_at_location_step` y
    `test_multiday_switch_by_text_at_last_dive_step`, ya no existen — se
    perdieron en el refactor). Hallazgo en vivo (batería sintética contra
    PRE, 2026-08-26, conv 429): sin un chequeo real, "somos 5 para buceo" →
    "en realidad revisamos y somos 4" (corrección de grupo DENTRO de una
    reserva que se está armando, ninguna reserva existe todavía) disparaba
    el flujo `modify_headcount` recién añadido, como si "somos 4" hablara
    de una reserva YA hecha. Fix: además del chequeo legacy (por si algún
    step MIXED_* vuelve a usarse), se considera "construyendo activamente"
    en cuanto el núcleo YA tiene una actividad detectada, un slot pendiente,
    o algo en el carrito — cualquiera de esas tres señales significa que
    la reserva en curso es nueva, no una ya existente."""
    return bool(
...
# `_GROUP_RECOMPOSE_RE` (arriba, código legacy nunca conectado al núcleo
# actual tras el refactor de Fase 4), esta NO exige que "en realidad" preceda
# INMEDIATAMENTE a "somos": tolera relleno intermedio ("revisamos y", "lo
# pensamos y", "checamos y") — hallazgo en vivo (batería sintética contra
# PRE, 2026-08-26, lote 4) de que el fraseo real de una corrección casi
# nunca es tan limpio. Deliberadamente estrecho (exige el cue léxico
...
async def _route_message_inner(state: ConversationState, message: str) -> str:
    ...
    # Hallazgo (batería sintética contra PRE, 2026-08-26, lote 4): "¿puedo
    # tomarme una cerveza antes de bucear?" y "soy alérgico a los mariscos,
    # es un problema?" escalaban como `medical_questions` — el LLM de
    # sensitive_topic los interpreta como un tema médico/de sustancias,
    # pero son preguntas de POLÍTICA plana con respuesta ya conocida en el
    # catálogo (`no_alcohol_policy`, `food_policy`: veganos/vegetarianos/
    # celíacos reciben arroz con vegetales, cualquier alergia alimentaria se
    # avisa antes del tour). Escalar es peor que responder: le hace perder
    # tiempo al cliente por algo que el bot ya sabe con certeza. Se
    # comprueba ANTES de cualquier gate de seguridad (misma prioridad que
    # el link roto) para no dejar que ninguno de los dos se adelante.
    # Acotado a mención EXPLÍCITA de un alérgeno alimentario conocido del
    # catálogo (marisco/gluten/nueces/maní/lactosa) — una alergia sin ese
    # contexto ("tengo alergias severas, es peligroso bucear?") sigue yendo
    # al escalado médico normal, sin tocar.
    ...
    if (
        ...
        # "¿abren el 25 de diciembre?" caía en esta misma respuesta genérica
        # ("las salidas son diarias, siempre hay disponibilidad") — FALSA
        # para esos dos días concretos: `policies.json["closed_days"]` dice
        # explícitamente "Solo cerramos el 25 de diciembre y el 1 de enero".
        # Se comprueba ANTES del canned genérico para no darle al cliente
        # información incorrecta sobre un día que sí está cerrado.
