...
Estado: **Fase B y Fase C implementadas en local (2026-07-17), pendiente de desplegar a PRE y probar en vivo. A sin empezar.** Este documento es la referencia técnica para implementar, probar y desplegar a PRE las 3 fases acordadas, en orden **B → C → A**.
...
- **Aún pendiente repetir la prueba de la Fase B en vivo con el guion completo**, ahora que el bloqueo de botones ya no debería interponerse — y ahora también con la Fase C activa, que debería capturar el detalle del padre incluso si la Fase B tarda en disparar.
