...
- **2026-07-17 — primer intento de prueba en vivo en PRE bloqueado por un bug NO relacionado, ya arreglado**: al probar el guion, la conversación entró en el flujo guiado (certificación → grupo → ubicación → plan → "¿hace más de 2 años que buceaste?") y ahí se quedó atascada — cualquier pregunta libre en ese paso (y en el de refresher) devolvía "no te entendí" sin llegar nunca a RAG. Causa: `supervisor.py` forzaba esos 2 pasos de vuelta al árbol de decisión asumiendo que el handler interpreta texto libre, cosa falsa para esos 2 en concreto. Arreglado en v0.20.18 (ver `docs/HISTORY.md`) — ahora esos pasos solo van al árbol si el mensaje resuelve como respuesta al botón; si no, caen a RAG normal.
- **2026-07-17 — segundo intento en vivo: el detalle del padre nunca llegó a recordarse (fallback de asesor)**: se comprobó (simulando la conversación exacta en local) que la cantidad de mensajes NO era el problema — `decision_tree.process_message()` guarda absolutamente todo en `state.history`, incluidos los clics de botón, así que para el turno de la pregunta de seguimiento ya había mensajes de sobra para que la Fase B disparase. La causa real más probable: "mi padre tiene la rodilla operada, evitar planes físicos" no encaja en ninguna de las 5 categorías fijas de `remembered_facts`, así que ni siquiera quedó registrado ahí — exactamente el hueco que la Fase C existe para cerrar. Se añadió también un log de éxito en `conversation_summarizer.py` (antes solo logueaba fallos) para poder auditar en los logs de PRE si el resumen de la Fase B se genera correctamente cuando corresponda.
- **2026-07-17 — Fase C implementada (local, no desplegada)**: motivada directamente por el caso real de arriba. Tests escritos primero (TDD): `tests/test_remembered_notes.py` (6 tests), confirmados en rojo (el campo "notes" se perdía por completo). Cambios: (1) nueva propiedad `notes` (array de strings) en el schema de la tool `remember` (`orchestrator.py`); (2) `_persist_remembered` (`supervisor.py`) ahora ACUMULA `notes` en vez de sobrescribir como las 5 claves fijas — con deduplicación y tope de `_MAX_REMEMBERED_NOTES=8` (se quedan las más recientes); (3) `_build_extra_context` renderiza `notes` como lista de viñetas aparte, con su propia cabecera, en vez de aplanarlas en la frase de una línea que usan las 5 claves fijas. Suite: 1642 passed, 15 skipped (mismos fallos preexistentes). **Pendiente**: desplegar a PRE y confirmar en vivo con el escenario real (mensaje del padre + pregunta de seguimiento) que el detalle sobrevive ahora.
- **2026-07-17 — guion completo probado en vivo contra PRE real (SSH + `live_battery_driver.py`, sin pasar por el widget)**: 8/9 puntos correctos en el primer intento, incluida la memoria — el bot recordó "rodilla operada" 9 turnos después y recomendó snorkel (Fase B y/o C funcionando). El único fallo ("¿hay descuento por grupo?") no era de memoria sino un bug de indexación real en `scripts/load_embeddings.py` (claves equivocadas dejaban el chunk de descuentos en blanco desde hacía años) + flakiness del verificador de grounding al calcular precios derivados — ambos arreglados y confirmados en vivo (v0.20.22/v0.20.23, ver `docs/HISTORY.md`).
- **2026-07-17 — Fase A implementada (TDD, tests en rojo antes de implementar)**: `tests/test_history_window.py` (5 tests). Nuevo campo `settings.history_window_size` (default 24, override por env var `HISTORY_WINDOW_SIZE`) y `settings.history_retrieval_enrichment_window` (default 10) en `src/config.py`, sustituyendo los literales `12`/`6` repetidos en `rag_agent.py` (respuesta LLM, contexto de grounding, enriquecimiento de retrieval) y `orchestrator.py` (decisión). `conversation_summarizer._SUMMARY_TRIGGER_EVERY` ahora se deriva del mismo `settings.history_window_size` en vez de un `12` hardcodeado aparte, cerrando la dependencia B↔A señalada en el diseño original. Suite: 1656 passed, 15 skipped.
