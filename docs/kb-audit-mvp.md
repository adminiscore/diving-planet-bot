...
| Precios actualizados | Parcial | `services.json` y `pricing.json` tienen precios confirmados para todos los tours y paquetes principales (2/5/7/9 inmersiones, minicurso, snorkel, bautismo, Bubble Makers). Ya no hay entradas `precio_a_definir`. | Pendiente: precios exactos de cursos PADI (OW/Advanced/Rescue/DM/Especialidades), política de precios para colombianos, y paquetes multi-persona | Confirmar precios de cursos PADI con el owner; política colombiana bloqueada a la espera de confirmación |
...
| Preguntas frecuentes | Cubierto | `faqs.json` cubre 130+ entradas: ubicación, horarios, grupos mixtos, edad, vuelos, recogidas, clima, Bubble Makers ($187 USD), Nitrox ($10/tanque), mascarillas con graduación (dioptrías 2/3/4), equipos infantiles, cursos interrumpidos por clima, idiomas del instructor, certificación eCard PADI, combinación de cursos, etc. (rondas 1-4 de Q&A incorporadas) | Algunas respuestas mezclan varios temas | Separar FAQs importantes en respuestas más concretas con el tiempo |
...
| Hoteles, recogidas y traslados | Parcial | `policies.json`, `faqs.json` y árbol incluyen selector de isla/hotel (12 islas). Confirmados con el owner: San Pedro y Cocoliso tienen muelle propio (recogida confirmada). Pao Pao expresamente incierto (bot no sobreafirma). | Faltan ~10 hoteles prioritarios sin validar por el owner. Sin lista completa el bot no puede confirmar recogida para hoteles no conocidos. | Pendiente: matriz hotel → recogida (sí/no) con el owner |
| Descuentos | Parcial | `discounts.json` actualizado (ronda 4 Q&A): descuento online/directo (10%), grupo ≥5 personas (10%, no aplica a cursos PADI, no automático), segundo día (15%). Combinaciones y restricciones documentadas. | Pendiente: descuento para colombianos/residentes (política no confirmada por el owner tras 4 rondas). El bot informa que existe y escala a asesor. | Cerrar con owner la política de descuento colombiano |
...
| Datos para cualificar leads | Parcial | `build_lead_summary` en `src/agents/lead_summary.py` genera resumen estructurado (servicio, grupo, ubicación, idioma, COP/USD) que se envía a Chatwoot al escalar | El resumen existe pero no captura fecha de reserva ni datos de contacto | Añadir campo de fecha y nombre/contacto al resumen antes de PRE |
