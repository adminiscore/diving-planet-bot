...
| Precios actualizados | Requiere confirmación | `services.json` tiene precio para `2_dives_1_day`; `pricing.json` tiene muchos `precio_a_definir` | El bot no puede inventar precios; puede responder incompleto | Confirmar precios oficiales por servicio, nacionalidad, salida desde Cartagena/islas, cursos y paquetes |
...
| Requisitos por curso | Parcial | `services.json` y flujo de cursos en el árbol | Open Water tiene algo de detalle; Advanced, Rescue y Divemaster están poco definidos | Completar duración, prerrequisitos, edad mínima, qué incluye, teoría, vuelos y certificación |
...
| Pagos | Falta / Parcial | `escalation_rules.json` menciona problemas de pago | Falta medios de pago, depósito, moneda, links, tarjetas extranjeras, transferencias | Crear política de pagos antes de que el bot responda detalles |
...
Completar primero las secciones de **precios/descuentos**, **reserva/pago** y **cancelaciones/cambios**. Son las áreas con más impacto comercial y mayor riesgo si el bot responde mal.
