...
- Conversation context / orchestrator (0.16.5, Fase 1 de `docs/conversation-orchestrator-plan.md`): `_build_extra_context` (`supervisor.py`) ahora añade el carrito completo (`state.mixed_cart`) y el step actual; el RAG ve `history[-12:]` (antes 6). PENDIENTE para Gadea/Gonzalo: Fase 0 (reindexar+reiniciar para desplegar los fixes de hoy), Fase 2 (orquestador con tool-calling — `src/agents/orchestrator.py` nuevo + dispatcher en supervisor para que el texto libre modifique el árbol: set_location, start_booking, cart_action, escalate...), Fase 3 (config `openai_model="gpt-4o"`), Fase 4 (tests). El plan detallado y el handoff exacto están en `docs/conversation-orchestrator-plan.md`.
