...
3. ✅ **HECHO (2026-07-28, Claude — commit `b9494f3`)**: borrados `process_message` + `_route` + **~117 métodos/funciones muertos en cascada** (los ~70 handlers `_handle_*` + helpers) con el eliminador iterativo, PROTEGIENDO la base compartida + los 9 símbolos de `cart_render` + `set_quick_replies`. **decision_tree.py: 8384 → 4321 líneas.** 0 handlers `_handle_*` restantes; los 9 símbolos intactos; **smoke del render OK** (cart_booking_blocks con link real, goto_final_summary 545 chars con precio+link); suite 1410. También borrado `_LOCATION_ANSWER_RE` (constante muerta) + 2 tests legacy + `_go_back_one_step`. supervisor.py: 3181 líneas.
4. **PENDIENTE — Módulo `orchestrator` + pasos `Step.MIXED_*`**:
   - `orchestrator` SIGUE vivo: `supervisor._apply_orchestrator_decision` (línea ~1864, despacha `TOOL_*`) se llama desde 2 funciones (~2161/2235) que quedan protegidas por refs de TEST (conftest/test_conversations/test_history_window/test_llm_extractor/test_owner_conversations/test_rag_safety parchean/usan `orchestrator`). Hay que trazar si esas 2 funciones son runtime-dead (solo tests) → si sí, borrar la cadena + el módulo `orchestrator` + retirar/adaptar sus tests. Si alguna es runtime-live, el orchestrator no es removible aún (revisar).
   - `Step.MIXED_*`: aún **~130 referencias** en src, casi todas en constantes/guards vestigiales del supervisor (`_MIXED_FLOW_STEPS`, `_CERT_COMPANION_SPLIT_STEPS`, y los `if state.step not in _MIXED_FLOW_STEPS:` que con el núcleo on son SIEMPRE-True → no-ops). Hay que limpiar esos constantes+guards primero (ver paso 5); solo entonces los valores del enum quedan huérfanos y borrables.
5. Quitar el flag `conversational_core` + gate en supervisor + `test_flag_off_core_not_engaged` + los guards/constantes `_MIXED_FLOW_STEPS` vestigiales + fixtures que parchean `detect_language_llm` + `CONVERSATIONAL_CORE` en `docker-compose.vps.yml`.
